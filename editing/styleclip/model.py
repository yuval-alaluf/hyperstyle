...
class ToRGB(nn.Module):
    ...
    def forward(self, input, style, skip=None, weights_delta=None, input_is_stylespace=False):
        out, style = self.conv(input, style, weights_delta, input_is_stylespace=input_is_stylespace)
        out = out + self.bias
...
class Generator(nn.Module):
    ...
    def forward(
        ...
        for conv1, conv2, noise1, noise2, to_rgb in zip(
            ...
            if not input_is_stylespace:
                out, out_style1 = conv1(out, latent[:, i], noise=noise1, weights_delta=weights_deltas[weight_idx])
                out, out_style2 = conv2(out, latent[:, i + 1], noise=noise2, weights_delta=weights_deltas[weight_idx + 1])
                skip, rgb_style = to_rgb(out, latent[:, i + 2], skip, weights_delta=weights_deltas[weight_idx + 2])
                ...
                out, out_style1 = conv1(out, latent[i], noise=noise1, weights_delta=weights_deltas[weight_idx], input_is_stylespace=input_is_stylespace)
                out, out_style2 = conv2(out, latent[i + 1], noise=noise2, weights_delta=weights_deltas[weight_idx + 1], input_is_stylespace=input_is_stylespace)
                skip, rgb_style = to_rgb(out, latent[i + 2], skip, weights_delta=weights_deltas[weight_idx + 2], input_is_stylespace=input_is_stylespace)
